...
class QueueConfigRequest(BaseModel):
    """Payload to configure the quiz queue for a session."""

    session_id: str
    """The queue session ID generated by the frontend on first load."""

    mode: str
    """Queue mode: 'random', 'shuffle', 'sequential', 'mini-batch', or 'weighted'."""

    batch_size: int = 10
    """Number of items per mini-batch. Only used in mini-batch mode."""

    source: str = "fallback"
    """The active quiz source. Used to validate mode availability."""


class RecordResultRequest(BaseModel):
    """Payload to record a guess result for weighted queue adjustment."""

    session_id: str
    """The queue session ID."""

    characters: str
    """The compound that was shown."""
...
class ResetQueueRequest(BaseModel):
    """Payload to reset queue state for a session."""

    session_id: str
    """The queue session ID to reset."""
